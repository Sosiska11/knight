...
conn.on('ready', async () => {
  ...
  try {
    ...
    console.log('\n--- Cleaning up old port 8443 rules ---');
    ...
    // 3. Set up iptables port forwarding rules for port 80
    console.log('\n--- Configuring iptables rules for port 80 ---');
    // Clear any existing matching DNAT rules for port 80 to avoid duplicates
    ...
    // Add DNAT rule for port 80
    await executeCommand(conn, `iptables -t nat -A PREROUTING -p tcp --dport 80 -j DNAT --to-destination ${mainVpsIp}:8443`);
    // Add MASQUERADE rule for port 8443 (destination port of forwarded traffic)
    // Note: MASQUERADE matches on the outgoing packet after DNAT, which has dport 8443
    await executeCommand(conn, `iptables -t nat -D POSTROUTING -p tcp -d ${mainVpsIp} --dport 8443 -j MASQUERADE 2>/dev/null || true`);
    ...
    console.log('\n🎉 Russian Transit VPS successfully configured to forward port 80 to 8443!');
    conn.end();
  } catch (err) {
