...
const config = {
  host: '79.137.162.56',
  port: 16605, // Connects to SSH currently listening on port 22 (NAT 16605 -> 22)
  username: 'root',
...
echo "=== 1. Configuring sslh with correct --tls flag ==="
...
DAEMON_OPTS="--user sslh --listen 0.0.0.0:22 --ssh 127.0.0.1:2222 --tls 141.11.197.6:8443 --pidfile /var/run/sslh/sslh.pid"
MIGRATE_EOF
...
`;
    ...
    await uploadFileSFTP(conn, '/tmp/migrate-ssh-fixed.sh', migrationScript);
    await executeCommand(conn, 'chmod +x /tmp/migrate-ssh-fixed.sh');
    
    // Run script
    console.log('\n--- Running migration script ---');
    const runRes = await executeCommand(conn, '/tmp/migrate-ssh-fixed.sh');
