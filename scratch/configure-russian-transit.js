...
conn.on('ready', async () => {
  ...
  try {
    ...
    // 2. Set up iptables port forwarding rules
    console.log('\n--- Configuring iptables rules ---');
    // Clear any existing matching DNAT rules to avoid duplicates
    ...
    // Add DNAT rule for port 8443
    await executeCommand(conn, `iptables -t nat -A PREROUTING -p tcp --dport 8443 -j DNAT --to-destination ${mainVpsIp}:8443`);
    // Add MASQUERADE rule for port 8443
    ...
    await executeCommand(conn, 'apt-get update && DEBIAN_FRONTEND=noninteractive apt-get install -y iptables-persistent || echo "Skipping persistent package installation"');
    ...
    console.log('\n🎉 Russian Transit VPS successfully configured!');
    conn.end();
  } catch (err) {
