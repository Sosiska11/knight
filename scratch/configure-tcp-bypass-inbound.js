...
async function run() {
  ...
  // 1. Change network protocol to tcp
  streamSettings.network = "tcp";
  
  // 2. Remove grpcSettings
  delete streamSettings.grpcSettings;

  // 3. Set Reality target and serverNames
  if (streamSettings.realitySettings) {
    streamSettings.realitySettings.target = "max.ru:443";
  ...
  const payload = {
    ...
    port: 8443, // Port updated to 8443
  ...
  console.log('Updating inbound 2 settings to TCP Reality on port 8443...');
