import fs from 'fs';
import { spawn } from 'child_process';

const config = {
  "log": {
    "loglevel": "info"
  },
  "inbounds": [
    {
      "port": 12345,
      "listen": "127.0.0.1",
      "protocol": "socks",
      "settings": {
        "auth": "noauth",
        "udp": true
      }
    }
  ],
  "outbounds": [
    {
      "protocol": "vless",
      "settings": {
        "vnext": [
          {
            "address": "cdn.node-ping-stat.ru",
            "port": 443,
            "users": [
              {
                "id": "985e730a-42aa-441f-88a0-d9223e6da8b1",
                "encryption": "none"
              }
            ]
          }
        ]
      },
      "streamSettings": {
        "network": "xhttp",
        "security": "tls",
        "tlsSettings": {
          "serverName": "cdn.node-ping-stat.ru"
        },
        "xhttpSettings": {
          "path": "/knight-down"
        }
      }
    }
  ]
};

const filePath = 'scratch/test-xr-run-no-mode.json';
fs.writeFileSync(filePath, JSON.stringify(config, null, 2));

const xrayPath = 'C:\\Program Files\\FlyFrogLLC\\Happ\\core\\xray.exe';
console.log('Starting xray with no mode...');
const child = spawn(xrayPath, ['run', '-c', filePath]);

let logOutput = '';
child.stdout.on('data', (data) => {
  const str = data.toString();
  logOutput += str;
  process.stdout.write(str);
});

child.stderr.on('data', (data) => {
  const str = data.toString();
  logOutput += str;
  process.stderr.write(str);
});

setTimeout(() => {
  console.log('Terminating xray...');
  child.kill();
  console.log('Terminated.');
  process.exit(0);
}, 3000);
